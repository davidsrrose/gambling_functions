...
coin_flip_money_gram <- function(n = 1000){
  ...
  distribution_plot <- ggplot(profit_df, aes(x=profit,y=probability,fill=positive,pattern=positive)) +
    ...
    #mean label 
    annotate(geom = "text", label = paste("E(profit)"),x = average_profit+5, y = y_axis_max ,hjust="left",color = "darkgreen", size = 4) +
    ...
    annotate(geom = "text", x = x_axis_max*.8, y = y_axis_max, hjust = "left",vjust=1,size=4, #...
  ...
  filename = gsub(" ","",paste("coin_flip_money_gram_n_",n,"_flips.png"))
