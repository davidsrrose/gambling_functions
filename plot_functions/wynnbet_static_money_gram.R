#Creates a density histogram (but its actually a bar chart of probability by profit)

#inputs
#n = number of simulations/times we run the promotion being plotted
#strategy = the strategy we want to use for the promotion - data pulls from the wynnbet static strategy dataframe
...
#none
#-----------------------------------------------------------------------------------------------------------------------------------------------
wynnbet_static_money_gram <- function(n = 10000,strategy = "static0"){
  ...
  profit_df$bar_label_x[which(profit_df$bar_label_justification =="right")] <- #...
                      profit_df$profit[which(profit_df$bar_label_justification =="right")]+bar_width/2-10
  ...
  distribution_plot <- ggplot(profit_df, aes(x=profit,y=probability,fill=positive,pattern=positive)) +
    ...
    geom_segment(aes(x = average_profit, y = 0, xend = average_profit, yend = e_profit_bar_label_probablity-y_axis_max/100), #...
                 linetype = "dashed", col = "darkgreen",size=.8,alpha=.8) +
    #add vertical mean line top (around vertical mean line bar label)
    geom_segment(aes(x = average_profit, y =  e_profit_bar_label_probablity+y_axis_max/12, xend = average_profit, yend = Inf),#...
                 linetype = "dashed", col = "darkgreen",size=.8,alpha=.8) +
    #mean label 
  ...
  filename = gsub(" ","",paste("wynnbet_static_money_gram_",strategy,".png"))
  
  #save png file
  ggsave(file=filename,path = "plots", distribution_plot,width = 12,height=7,units="in")
