#generates and saves a scatter plot of expected profit of wynnbet bonus bets bet across different odds
#this gives a good visual indication of the optional +odds to play free bets at by viewing the EV of each of the 20+ odds

#inputs:
#n - the number of monte carlo simulations we want to make per odd we loop through (-150 to 2000ish)
...
odds_ev_scatter_plot <- function(n=1000){
  ...
  for (i in 1:(odds_looped)){
    for (bonus_bets in 0:5){
      e_profit_df[i,bonus_bets+2] <- wynnbet_bonus_only(bonus_odds=e_profit_df$odds[i],bonus_bets = bonus_bets,n=n)
    }
  ...
  ev_scatter_plot <- ggplot(e_profit_df, aes(x=odds,y=value,color=variable)) +
    ...
    #title
    ggtitle(paste("Expected Profit - across 6 levels of wynnbet bonus convert vs bet")) +
  ...
  #save png file
  ggsave(file=filename,path = "plots", ev_scatter_plot,width = 12,height=7,units="in")
}